...
func (mg *Integration) GetTerraformResourceType() string {
	return "elasticstack_fleet_integration"
}
...
func (mg *IntegrationPolicy) GetTerraformResourceType() string {
	return "elasticstack_fleet_integration_policy"
}
...
// LateInitialize this IntegrationPolicy using its observed tfState.
// returns True if there are any spec changes for the resource.
func (tr *IntegrationPolicy) LateInitialize(attrs []byte) (bool, error) {
	params := &IntegrationPolicyParameters{}
	if err := json.TFParser.Unmarshal(attrs, params); err != nil {
		return false, errors.Wrap(err, "failed to unmarshal Terraform state parameters for late-initialization")
	}
	opts := []resource.GenericLateInitializerOption{resource.WithZeroValueJSONOmitEmptyFilter(resource.CNameWildcard)}
